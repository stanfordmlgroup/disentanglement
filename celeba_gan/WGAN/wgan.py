...
class GoodGenerator(nn.Module):
    def __init__(self, dim=DIM,output_dim=OUTPUT_DIM):
        ...
        if False:
            #using cifar:
            self.ln1 = nn.Linear(64, 4*4*4*self.dim)
            self.rb1 = None
    ...
    def forward(self, input, im_reshape=True):
...
class GoodDiscriminator(nn.Module):
    def __init__(self, dim=DIM):
        ...
        if True:
            self.ln1 = nn.Linear(4*4*8*self.dim, 1)# for cifar
        else:
    ...
    def forward(self, input):
        ...
        output = self.rb3(output)
        if True:
            output = output.view(-1, 4*4*8*self.dim) #for cifar
        else:
            output = self.rb4(output)
