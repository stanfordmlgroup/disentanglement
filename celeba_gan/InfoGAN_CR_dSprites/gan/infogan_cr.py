...
if __name__ == "__main__":
    ...
    checkpoint_dir = f"./test_bs{args.bs}/checkpoint"
    if not os.path.exists(checkpoint_dir):
        os.makedirs(checkpoint_dir)
    sample_dir = f"./test_bs{args.bs}/sample"
    if not os.path.exists(sample_dir):
        os.makedirs(sample_dir)
    time_path = f"./test_bs{args.bs}/time.txt"
    metric_path = f"./test_bs{args.bs}/metric.csv"
    epoch = 28
    batch_size = args.bs
    vis_freq = 200
    vis_num_sample = 10
