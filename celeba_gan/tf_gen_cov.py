...
# original imports from gen_cov.py
import os
import gs
import numpy as np
...
def interpolate(args, z2s):
    ...
    # uniform sampling for infogans
    # fixed_zz = sample_noise(args.viz_batch_size, args.nz, args.device)
    fixed_zz = np.random.uniform(size=(args.viz_batch_size, args.nz))
    fixed_zs = []
    n_view = 1
    for dim in range(args.nz):
        fixed_z = np.tile(np.random.uniform(size=(n_view, args.nz)), (args.viz_batch_size, n_view)).astype(np.float32)
        fixed_z[:, dim] = np.linspace(0.01, 0.99, args.viz_batch_size)
    ...
    for iz, fixed_z in enumerate(fixed_zs):
        with torch.no_grad():
            fake = decoder.sample_from(fixed_z)
    ...
    for iviz in range(10):
        ...
        fixed_zs = []
        for dim in range(args.nz):
            fixed_z = np.tile(np.random.uniform(size=(1, args.nz)), (args.viz_batch_size, 1)).astype(np.float32)
            fixed_z[:, dim] = np.linspace(0.01, 0.99, args.viz_batch_size)
            fixed_zs.append(fixed_z)
